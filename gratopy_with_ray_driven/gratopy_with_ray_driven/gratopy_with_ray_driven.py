...
# Class attribute corresponding to which discretization framework is used
PIXEL = 1
RAY = 2
...
def forwardprojection(img, projectionsetting, sino=None,method=PIXEL , wait_for=[]):
    """
    ...
    :param method: gratopy.PIXEL (=1) is the default or gratopy.RAY (=2).
		Decides which discretization method is used.
    :type method: :class:`int`
        default :obj:`PIXEL-driven method via gratopy.PIXEL`   
    ...
    if method == PIXEL:
        ...
        function = projectionsetting.forwardprojection_ray
    else:
        raise ValueError("Unknown method,  the input"+ str(method)+
                             + " is must be PIXEL(=1) or Ray(=2)")
    function(sino, img, projectionsetting, wait_for=wait_for)
    return sino



def forwardprojection_expo(img, projectionsetting, sino=None,mu=1., wait_for=[]):
    """
    Performs the forward projection (either for the Radon or the
    fanbeam transform) of a given image using the given projection
    settings.
    ...
    # perform projection operation
    function = projectionsetting.forwardprojection_expo

    function(sino, img, projectionsetting,np.float32(mu), wait_for=wait_for)
    return sino


def backprojection(sino, projectionsetting, img=None, method=PIXEL, wait_for=[]):
    """
    ...
    *C* and *F* contiguous
    ...
    if method == PIXEL:
        ...
        function = projectionsetting.backprojection_ray
    else:
        raise ValueError("Unknown method,  the input"+ str(method)+
                             + " is must be PIXEL(=1) or Ray(=2)")
...
def radon_ray(sino, img, projectionsetting, wait_for=[]):
    """
    Performs the Ray-Driven Radon transform of a given image using the
    given **projectionsetting**.
    ...
    # choose function with appropriate dtype
    function = projectionsetting.functions_ray[(dtype,
                                            sino.flags.c_contiguous,
                                            img.flags.c_contiguous)]
...
def radon_expo(sino, img, projectionsetting, mu,wait_for=[]):
    """
    Performs the Radon transform of a given image using the
    given **projectionsetting**.
    ...
    # choose function with appropriate dtype
    function = projectionsetting.functions_expo[(dtype,
                                            sino.flags.c_contiguous,
                                            img.flags.c_contiguous)]
    ...
    myevent = function(sino.queue, sino.shape, None,
                       sino.data, img.data, ofs_buf,
                       geometry_information, np.float32(mu),
                       wait_for=img.events+sino.events+wait_for)
...
def radon_ray_ad(img, sino, projectionsetting, wait_for=[]):
    """
    Performs the Radon backprojection of a given sinogram using
    the given **projectionsetting**.
    ...
    # choose function with appropriate dtype
    function = projectionsetting.functions_ray_ad[(dtype,
                                               img.flags.c_contiguous,
                                               sino.flags.c_contiguous)]
...
def fanbeam_expo(sino, img, projectionsetting,mu, wait_for=[]):
    """
    Performs the fanbeam transform of a given image using the
    given **projectionsetting**.
    ...
    # choose function with appropriate dtype
    function = projectionsetting.functions_expo[(dtype, sino.flags.c_contiguous,
                                            img.flags.c_contiguous)]
...
class ProjectionSettings():
    ...
    def __init__(self, queue, geometry, img_shape, angles,
        ...
        if self.geometry in [RADON, PARALLEL]:
            ...
            self.forwardprojection_ray = radon_ray
            self.forwardprojection_expo = radon_expo

            self.backprojection = radon_ad
            self.backprojection_ray = radon_ray_ad
            ...
            self.functions_ray = {(float32, 0, 0): self.prg.radon_ray_float_ff,
                              (float32, 1, 0): self.prg.radon_ray_float_cf,
                              (float32, 0, 1): self.prg.radon_ray_float_fc,
                              (float32, 1, 1): self.prg.radon_ray_float_cc,
                              (float64, 0, 0): self.prg.radon_ray_double_ff,
                              (float64, 1, 0): self.prg.radon_ray_double_cf,
                              (float64, 0, 1): self.prg.radon_ray_double_fc,
                              (float64, 1, 1): self.prg.radon_ray_double_cc}

            self.functions_expo = {(float32, 0, 0): self.prg.radon_expo_float_ff,
                              (float32, 1, 0): self.prg.radon_expo_float_cf,
                              (float32, 0, 1): self.prg.radon_expo_float_fc,
                              (float32, 1, 1): self.prg.radon_expo_float_cc,
                              (float64, 0, 0): self.prg.radon_expo_double_ff,
                              (float64, 1, 0): self.prg.radon_expo_double_cf,
                              (float64, 0, 1): self.prg.radon_expo_double_fc,
                              (float64, 1, 1): self.prg.radon_expo_double_cc}
            ...
            self.functions_ray_ad = {(float32, 0, 0): self.prg.radon_ray_ad_float_ff,
                                 (float32, 1, 0): self.prg.radon_ray_ad_float_cf,
                                 (float32, 0, 1): self.prg.radon_ray_ad_float_fc,
                                 (float32, 1, 1): self.prg.radon_ray_ad_float_cc,
                                 (float64, 0, 0): self.prg.radon_ray_ad_double_ff,
                                 (float64, 1, 0): self.prg.radon_ray_ad_double_cf,
                                 (float64, 0, 1): self.prg.radon_ray_ad_double_fc,
                                 (float64, 1, 1): self.prg.radon_ray_ad_double_cc}
        ...
        if self.geometry in [FAN, FANBEAM]:
            ...
            self.forwardprojection_expo = fanbeam_expo
            ...
            self.functions_expo = {(float32, 0, 0): self.prg.fanbeam_expo_float_ff,
                              (float32, 1, 0): self.prg.fanbeam_expo_float_cf,
                              (float32, 0, 1): self.prg.fanbeam_expo_float_fc,
                              (float32, 1, 1): self.prg.fanbeam_expo_float_cc,
                              (float64, 0, 0): self.prg.fanbeam_expo_double_ff,
                              (float64, 1, 0): self.prg.fanbeam_expo_double_cf,
                              (float64, 0, 1): self.prg.fanbeam_expo_double_fc,
                              (float64, 1, 1): self.prg.fanbeam_expo_double_cc}
    ...
    def create_sparse_matrix_expo(self,mu, dtype=np.dtype('float32'), order='F'):
        """
        Creates a sparse matrix representation of the associated forward
        projection.
        ...
        if self.is_parallel:
            functions = {
                (np.dtype("float32"), 0): self.prg.single_line_radon_expo_float_ff,
                (np.dtype("float32"), 1): self.prg.single_line_radon_expo_float_cc,
                (np.dtype("float64"), 0): self.prg.single_line_radon_expo_double_ff,
                (np.dtype("float64"), 1): self.prg.single_line_radon_expo_double_cc
                }
        elif self.is_fan:
            functions = {
                (np.dtype("float32"), 0): self.prg.single_line_fan_expo_float_ff,
                (np.dtype("float32"), 1): self.prg.single_line_fan_expo_float_cc,
                (np.dtype("float64"), 0): self.prg.single_line_fan_expo_double_ff,
                (np.dtype("float64"), 1): self.prg.single_line_fan_expo_double_cc
                }
        # choose relevant function
        dtype = np.dtype(dtype)
        function = functions[(np.dtype(dtype), order == 'C')]
        ...
        # (arguments depend on geometry)
        if self.is_parallel:
            def projection_from_single_pixel(x, y, mu, sino=None, wait_for=[]):
                myevent = function(sino.queue, sino.shape, None,
                                   sino.data, np.int32(
                                       x), np.int32(y), np.float32(mu), ofs_buf,
                                   geometry_information,
                                   wait_for=sino.events+wait_for)
                sino.add_event(myevent)
        else:
            def projection_from_single_pixel(x, y, mu,sino=None, wait_for=[]):
                myevent = function(sino.queue, sino.shape, None,
                                   sino.data, np.int32(x), np.int32(
                                       y), np.float32(mu),ofs_buf, sdpd_buf,
                                   geometry_information,
                                   wait_for=sino.events+wait_for)
                sino.add_event(myevent)
        ...
        for x in range(Nx):
            ...
            for y in range(Ny):
                # compute projection for delta peaks in x,y and write onto sino
                projection_from_single_pixel(x, y,mu, sino)
                sinonew = sino.get()
                pos = pos_1(x, y)
