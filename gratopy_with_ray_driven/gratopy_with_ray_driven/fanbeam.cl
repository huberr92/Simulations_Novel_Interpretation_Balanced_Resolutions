...
//			values inside sino are altered to represent the computed
//                      fanbeam transform
__kernel void fanbeam_expo_\my_variable_type_\order1\order2(
    __global real *sino, __global real *img, __constant real8 *ofs,
    __constant real *sdpd, __constant real *Geometryinformation, const float mu) {
  // Extract geometric information
  size_t Ns = get_global_size(0);
  size_t Na = get_global_size(1);
  size_t Nz = get_global_size(2);
  ...
  real beam_distance = hypot(R*delta_x,(s-midpoint_det)*delta_xi*delta_x);
  ...
  for (int y = 0; y < Nyy; y++) {
    ...
    real expo_val = exp(mu*t*beam_distance);
    ...
    for (int x = xlowint; x <= xhighint; x++) {
      ...
      // Weight corresponds to distance of projected detector position
      // divided by the distance from the source
      real Weight = ((real)1. - fabs(s_midpoint_det - xi))*expo_val / t;
      ...
      t += dd.x;
      ss += d.x;
      expo_val = exp(mu*t*beam_distance);
    }
  }
...
__kernel void single_line_fan_expo_\my_variable_type_\order1\order2(
...
//    if (s==299)
//    {printf ("Decimals: %f, %f, %f\n", t, (s-midpoint_det)*delta_xi*delta_x,beam_distance);}
    ...
    // Weight corresponds to distance of projected detector position
    // divided by the distance from the source
    real Weight = ((real)1. - fabs(s - xi - midpoint_det)) / (R * t)*expo_val;
